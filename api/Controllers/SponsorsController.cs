...
[ApiController]
[Route("v{version:apiVersion}/sponsors")]
...
) : ControllerBase
{
    ...
    {
        var result = await sponsorService.SearchAsync(q, page, pageSize, includePrivateMeta: false, ct);
        return Ok(result);
    }
    ...
    {
        var dto = await sponsorService.GetBySlugAsync(slug, includePrivateMeta: false, ct);
        if (dto is null) return NotFound(new ApiError(404, "Sponsor not found", HttpContext.TraceIdentifier));
        return Ok(dto);
    }

    [HttpGet("/v{version:apiVersion}/events/by-slug/{eventSlug}/sponsors")]
    public async Task<IActionResult> GetEventSponsors(string eventSlug, CancellationToken ct = default)
    {
        ...
        var sponsors = await sponsorService.ParseEventViewSponsorsAsync(ev.Sponsors, includePrivateMeta: false);
        return Ok(sponsors);
    }
    ...
    public async Task<IActionResult> GetEvents(string slug, [FromQuery] string status = "upcoming", [FromQuery] int page = 1, [FromQuery] int pageSize = 20, CancellationToken ct = default)
    {
        ...
        var sponsor = await context.SponsorViews.AsNoTracking().FirstOrDefaultAsync(s => s.Slug == slug, ct);
        if (sponsor is null) return NotFound(new ApiError(404, "Sponsor not found", HttpContext.TraceIdentifier));

        var query = context.EventViews.AsNoTracking()
            .Where(e => e.Status == "Published")
            .Where(e => EF.Functions.JsonContains(e.Sponsors, $"[{{\"sponsorId\":\"{sponsor.SponsorId}\"}}]"));
