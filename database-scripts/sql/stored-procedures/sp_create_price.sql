DROP FUNCTION IF EXISTS sp_create_price(uuid, text, text, int, int, bool, uuid, uuid, int);
...
AS $$
    SELECT app.create_price(p_event_id, p_name, p_pricing_type, p_base_price_cents,
        p_per_attendee_cents, p_is_all_inclusive, p_fee_formulas_id, p_max_quantity);
$$;
