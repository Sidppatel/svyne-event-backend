...
BEGIN
    ...
    WHERE "ActorType" = 'Developer'
      AND "CreatedAt" < now() - (p_dev_days || ' days')::interval;
    GET DIAGNOSTICS v_count = ROW_COUNT; v_total := v_total + v_count;

    DELETE FROM audit_logs
    WHERE "ActorType" = 'Admin'
      AND "CreatedAt" < now() - (p_admin_days || ' days')::interval;
    GET DIAGNOSTICS v_count = ROW_COUNT; v_total := v_total + v_count;

    DELETE FROM audit_logs
    WHERE "ActorType" = 'System'
      AND "CreatedAt" < now() - (p_system_days || ' days')::interval;
    GET DIAGNOSTICS v_count = ROW_COUNT; v_total := v_total + v_count;
